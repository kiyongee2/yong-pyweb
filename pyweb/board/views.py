...
from board.models import Question, Answer, Category, Comment
...
def question_list(request):
    ...
    context = {'question_list': page_obj, 'kw': kw, 'categories': categories}
    return render(request, 'board/question_list.html', context)

def category_page(request, slug):
    current_category = Category.objects.get(slug=slug)
    filter_list = Question.objects.filter(category=current_category)
    question_list = filter_list.order_by('-create_date')
    categories = Category.objects.all()
    ...
    paginator = Paginator(question_list, 10)  # 1페이지당 10개씩 페이지처리
    ...
    context = {
        ...
        'question_create_url': reverse('board:cate_question_create', args=[slug]) #질문등록 url
    }
    return render(request, 'board/question_list.html', context)
...
def detail(request, question_id):
    ...
    categories = Category.objects.all()
    context = {'question': question, 'categories': categories}
    return render(request, 'board/detail.html', context)

# 질문 등록
@login_required(login_url='common:login')
def cate_question_create(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid(): # 폼이 유효성 검사를 통과했다면
            ...
            form.save() #실제 저장
            return redirect('board:category_page', slug=slug) # 질문 목록 페이지 이동
    else: #get 방식
